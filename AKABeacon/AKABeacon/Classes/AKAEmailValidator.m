                     ...
                     error:(NSError *__autoreleasing *)error
{
    ...
    if ([modelValue isKindOfClass:[NSString class]])
    {
        ...
        if (!result && error != nil)
        {
            *error = [AKABeaconErrors errorForInvalidEmailAddress:text withRegularExpression:EMAIL_REGEX];
        }
    }
    else
    {
        if (error != nil)
        {
            *error = [AKABeaconErrors errorForInvalidEmailAddressValueType:modelValue];
        }
    }
    return result;
}
