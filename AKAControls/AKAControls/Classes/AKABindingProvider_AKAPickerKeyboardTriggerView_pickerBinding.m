                       ...
                       @{ @"choices":
                          ...
                          @"textForOtherValue":
                              @{ @"required":        @NO,
                                 ...
                                 @"attributes":      @{},
                                 @"use":             @(AKABindingAttributeUseAssignValueToBindingProperty),
                                 @"bindingProperty": @"textForOtherValue"
                                 },
            ...
            forComponent:(NSInteger)component
{
    ...
    {
        result = self.textForUndefinedValue;
    }
    else if (row == self.rowForOtherValue)
    {
        result = self.textForOtherValue;
    }
    else
    {
        ...
        if (index >= 0 && index < self.choices.count)
        {
            ...
            NSString* choiceText;
            NSError* error;
            if ([self convertTargetValue:choice toSourceValue:&choiceText error:&error])
            {
                choice = choiceText;
            }
            else
...
{
    return self.textForUndefinedValue.length > 0;
}
...
{
    return self.textForOtherValue.length > 0;
}
